...
A user-provided component that stores raw events, enriched records, artifacts, checkpoints, and audit logs.
...
5. **Enrich** — core invokes AI provider through task-specific contracts.
6. **Policy check** — core validates model output, user policy, and persistence rules.
7. **Persist** — persistence implementation writes raw and enriched data transactionally where possible.
8. **Checkpoint** — adapter/persistence checkpoint is committed after successful processing.
9. **Observe** — logs, metrics, traces, and run summaries are emitted.

## 5. Ports and adapters

Shiyi core exposes three primary ports:
...
flowchart TB
  ...
  Core --> PersistencePort[Persistence Port]
  AdapterPort --> CustomAdapter[Custom Adapter]
  AIProviderPort --> CustomAI[Custom AI Provider]
  PersistencePort --> CustomStore[Custom Persistence]
```
...
- Shiyi does not require one default database.
