...
6. **Partial content is explicit.** Summary-only or partial records may exist as degraded fallback, but they must be marked source-neutrally with `content_depth` and derived `content_completeness`, and must not count as AI Weekly source-ready evidence. `content_completeness` is intentionally limited to `complete`, `partial`, and `summary_only`; only `complete` is decision-grade by default. Lower-level `content_depth` may keep more mechanical detail such as `full_page`, `feed_full_content`, `summary_only`, `partial`, or `blocked`, but downstream readiness should use the three-state completeness contract.
